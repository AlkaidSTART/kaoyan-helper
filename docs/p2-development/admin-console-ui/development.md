...
> 日期：2026-09-26
> 状态：P2-101 至 P2-107 已实现，lint/build/test 与冒烟验证通过（见 P3）

## 1. 原子任务

- [ ] P2-101 shadcn 初始化：components.json、utils/cn、globals.css 主题变量、安装基础组件。
- [x] P2-102 根布局与 `/` 重定向、`proxy.ts` 导航守卫。
- [x] P2-103 登录页（真实 API + 错误文案映射 + 登出）。
- [x] P2-104 管理布局：服务端会话校验 + Sidebar + Header + 用户菜单。
- [x] P2-105 概览页：统计卡 + 待审核预览 + 最近用户表。
- [x] P2-106 用户/题库/UGC/院校页面：表格、徽章、筛选工具条（视觉）、待接入动作 Toast。
...
- 无系统 Emoji；示例数据均有标识。- [x] - 管理后台 UI 开发
